...
def inoue_tau(l, z):
    ...
    obs_l = l * (1. + z)
    A, B = 1 + z, obs_l / ly_l
    tau_LAF_LS, tau_DLA_LS, tau_LAF_LC, tau_DLA_LC = np.zeros(
        len(l)), np.zeros(len(l)), np.zeros(len(l)), np.zeros(len(l))
    ...
    for x in inoue_coeffs:
        ...
        cond2 = (obs_l > x['lambda']) & (obs_l < x['lambda'] * (1 + z)
                                         ) & (obs_l >= 2.2 * x['lambda']) & (obs_l < 5.7 * x['lambda'])
    ...
    if z < 1.2:
        ...
        tau_LAF_LC[cond1] += (2.55e-2 * (A**1.6) * (B[cond1]**2.1)) + \
        ...
        tau_LAF_LC[cond1] += (5.22e-4 * (A**3.4) * (B[cond1]**2.1)) + \
            (0.325 * B[cond1]**1.2) - (3.14e-2 * B[cond1]**2.1)
        tau_LAF_LC[cond2] += (5.22e-4 * (A**3.4) * (B[cond2]**2.1)) + \
            (0.218 * B[cond2]**2.1) - (2.55e-2 * B[cond2]**3.7)
    ...
    if z < 2.0:
        ...
        tau_DLA_LC[cond1] = 0.634 + (4.7e-2 * A**3.0) - (1.78e-2 * (A**3.3) *
                                                         (B[cond1]**-0.3)) - (0.135 * B[cond1]**2.0) - (0.291 * B[cond1]**-0.3)
        tau_DLA_LC[cond2] = (4.70e-2 * A**3.0) - (1.78e-2 * (A**3.3) *
