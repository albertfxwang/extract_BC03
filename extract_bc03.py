...
class TemplateSED_BC03(object):
    ...
    def read_gpl(self):
        ...
        whose first column ('rest_wavelength') specifies the wavelength in Angström and subsequent
        ...
        dtype = [('rest_wavelength',float),]+[('spec{}'.format(i+1),float) for i in range(len(self.age))]
    ...
    def add_emlines(self):
        ...
        for x in self.sed.dtype.names[1:]:
            self.sed[x] = add_emission_lines(self.sed['rest_wavelength'], self.sed[x], self.Q[x], self.metallicity, self.units)

    def add_dust(self):

        for x in self.sed.dtype.names[1:]:
            if   self.dust == 'calzetti':
                self.sed[x] = self.sed[x] * np.exp(-calzetti(self.sed['rest_wavelength'], self.Av))
            elif self.dust == 'cardelli':
                self.sed[x] = self.sed[x] * np.exp(-cardelli(self.sed['rest_wavelength'], self.Av))

    def redshift_evo(self):

        self.sed['rest_wavelength'] *= (1+self.redshift)
        if self.igm:
            for x in self.sed.dtype.names[1:]:
                self.sed[x] = self.sed[x] * np.exp(-inoue_tau(self.sed['rest_wavelength'], self.redshift))
    ...
    def plot_sed(self,save=None):
        ...
        for age,x,c in zip(self.age,self.sed.dtype.names[1:],colors):
            ax.plot(self.sed['rest_wavelength'],self.sed[x],c=c,lw=1.5,alpha=0.8,label="Age = %.3f Gyr" % age)
